...
- **タスク開始時**: 対応するJIRAチケットのステータスを In Progress に更新する
- **タスク完了時**: JIRAチケットのステータスを Done に更新する
- **plans/ 作成時**: ファイル冒頭にJIRAチケット番号を記録する（例: `JIRA: PROJ-123`）
- **対象プロジェクト**: 未定（決まり次第ここに追記する）
- チケット番号が不明な場合は人間に確認する。勝手に推測しない。
